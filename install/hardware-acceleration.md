...
Outside Docker (bare-metal Linux, Windows), CUDA detection with the ONNX plugin additionally needs the CUDA 13 libraries and cuDNN 9 installed system-wide from [NVIDIA](https://developer.nvidia.com/cuda-downloads). In Docker, the `nvidia` image ships them and needs an NVIDIA driver 580 or newer; the `nvidia-cuda12` image keeps CUDA 12 for older drivers and pairs with the ONNX Legacy plugin. The `nvidia-tensorrt` image additionally ships the TensorRT runtime for the ONNX plugin's tensorrt provider, which roughly doubles detection throughput at the cost of about twice the VRAM; outside Docker, install TensorRT 10 yourself if you want it. The line between the two runs at Turing: GTX 16xx / RTX 20xx and newer use the ONNX plugin on the CUDA 13 images, GTX 900 and 10xx (Maxwell, Pascal, Volta) only work with the ONNX Legacy plugin on `nvidia-cuda12`, and RTX 50xx runs exclusively on the CUDA 13 side. Cards older than GTX 900 are not supported.
