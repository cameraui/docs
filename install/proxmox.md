...
An LXC can **share** the iGPU: `/dev/dri` stays available to the host and other containers at the same time, while a VM with PCIe passthrough monopolizes the GPU, and an LXC has less overhead. A VM is the better choice for NVIDIA cards (see below).
:::
...
The script creates the container without a root password, so the Proxmox console and SSH won't let you in. From the host shell:
...
Console and SSH work once you set a password:
...
- **Intel / AMD iGPU.** Set `FLAVOR=intel` (or `amd`). The script passes the GPU render node through the container into Docker, nothing else to configure. Details: [Hardware acceleration](/install/hardware-acceleration).
- **NVIDIA.** Use a **VM with PCIe passthrough** instead of an LXC. Inside it, follow the standard [Docker setup](/install/docker) with the `nvidia` flavor. An LXC would need the host driver and the container's user-space libraries in permanent version lockstep. The VM does monopolize the card. If other services on the host need the same GPU (Ollama, Jellyfin), see [NVIDIA in an LXC](#nvidia-in-an-lxc-experimental) below.
- **AI accelerators (Coral, Hailo).** Install the [host driver](/install/hardware-acceleration#check-your-host) on the Proxmox host, then pass the device node (`/dev/apex_0`, `/dev/hailo0`) into the container.
...
Recordings default to the container's own volume. For a bigger disk, NAS share or merged pool, hand that storage into the container and set the NVR's storage path to it.
...
A FUSE mount (such as a **mergerfs** pool merging several drives) does not propagate into an (unprivileged) container the way a normal filesystem does. If the FUSE mount isn't active on the host **before the container starts**, the container sees the empty mountpoint on the host's root disk instead of the pool. Recordings then land on the small OS disk and get rotated away within minutes.
...
After setting a custom storage path, check that the container sees the full volume:

```bash
pct exec <CTID> -- df -h /mnt/recordings
```

The reported size must match your real disk or pool. The small container root size means the mount didn't propagate. The NVR also logs its resolved storage and size on every start, and shows a warning in **Settings → Recordings** when the volume is unexpectedly small.

## Verify

On every start the container logs which devices actually arrived. See [Verify from the container](/install/hardware-acceleration#verify-from-the-container). The camera.ui UI is available at `https://<container-ip>:3443` (the script prints the IP at the end).
