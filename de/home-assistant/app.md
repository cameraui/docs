...
Beim **ersten Start** lädt das App den camera.ui-Server und das Standard-Plugin herunter, was eine Internetverbindung braucht und ein paar Minuten dauert.[^firstboot] Den ersten Boot kannst du im Tab **Log** des Apps mitverfolgen.

[^direct]: Das App hört auch direkt unter `https://<host>:3443` im Host-Netzwerk. Dieser Weg zeigt einmalig eine Warnung wegen des selbstsignierten Zertifikats.
...
Das App reicht GPU-, KI-Beschleuniger- und Codec-Geräte in den Container durch.
...
- **Hailo.** Das Hailo-Gerät (`/dev/hailo0`) wird durchgereicht, die Hailo-Runtime ist aber nicht Teil des Images.[^hailo]
...
Das Verzeichnis `/media` ist mit Schreibrechten ins App eingebunden. Mit dem Standard-`recordings_path` `/media/camera.ui` erscheinen die Aufnahmen des NVR im **Medien**-Browser von Home Assistant.
...
- **master_api_port.** Der HTTPS-API-Port des Masters. Optional. Standard ist `3443`.
