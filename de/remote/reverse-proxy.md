...
Hinter Apache, nginx, Traefik oder einem anderen Reverse-Proxy braucht camera.ui mehr als reines HTTP: Live-Video, Events und die Kameraliste laufen über WebSockets. Ein Proxy, der nur normale Anfragen weiterleitet, liefert eine Oberfläche, die lädt und leer bleibt.

## Was der Server erwartet

**Der Hauptport liefert HTTPS aus.** camera.ui lauscht standardmäßig auf Port `3443`, mit einem selbst erzeugten Zertifikat. Ein Proxy, der auf diesen Port zeigt, muss sich mit `https://` verbinden und die Zertifikatsprüfung abschalten, da es keinen öffentlichen Aussteller zum Prüfen gibt. Alternativ schaltest du einen [reinen HTTP-Port](#stattdessen-reines-http) ein.

**WebSocket-Upgrades müssen den Server erreichen.** Sie laufen über denselben Host und Port wie der Rest der App. Am einfachsten leitest du alles unter `/` samt Upgrades weiter.
...
Aktiviere `proxy`, `proxy_http`, `proxy_wstunnel`, `ssl` und `headers` und nutze dann diesen Virtual Host:
...
Die Option `upgrade=websocket` braucht Apache 2.4.47 oder neuer (`apachectl -v`). Auf älteren Versionen lässt du die Option weg und leitest die Upgrades stattdessen über `mod_rewrite`:
...
Das Rewrite-Ziel muss `wss://` sein. Mit `ws://` geht der Handshake unverschlüsselt an einen TLS-Port, Apache protokolliert `error reading status line from remote server`, und jeder WebSocket scheitert mit einem 502, während normale Seiten problemlos laden.
...
camera.ui kann einen zweiten Listener öffnen, der reines HTTP spricht und alles inklusive WebSockets an den Hauptport weiterreicht. Trage dazu einen Port in der `camera.ui.yaml` in deinem Speicherverzeichnis ein:

```yaml
insecurePort: 8081
```

In Docker kannst du stattdessen `CAMERA_UI_INSECURE_PORT=8081` setzen. Starte den Server neu, richte den Proxy dann auf `http://10.0.0.27:8081` und lass die `SSLProxy*`-Zeilen beziehungsweise `proxy_ssl_verify` weg.
...
Kameras zeigen einen Platzhalter, die Event-Zeile füllt sich nie, nichts aktualisiert sich. Das ist ein WebSocket-Problem, kein Anmeldeproblem. Filtere in den Entwicklertools des Browsers den Netzwerk-Tab nach WS und lade neu. Ein fehlgeschlagener Eintrag mit Status 502 oder 400 bestätigt es, das Fehlerprotokoll deines Proxys nennt den Grund.
