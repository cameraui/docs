...
Es gibt zwei Wege, eine Kamera hinzuzufügen: camera.ui sie **entdecken** lassen, oder sie **manuell** mit ihrer Stream-URL hinzufügen. Beides startet auf der **Kameras**-Seite.

## Bevor du startest

- Die Kamera ist im selben Netzwerk wie dein Server.
...
Die Kameras-Seite listet Geräte, die **entdeckt** wurden und bereit zum Hinzufügen sind. Eine Kamera erscheint hier, wenn camera.uis eingebaute Suche sie in deinem Netzwerk findet (ONVIF und weitere Protokolle), oder wenn ein [Plugin](/de/plugins/), das Kameras verwaltet, eine meldet. Bereits hinzugefügte Kameras erscheinen unter **Verbundene Kameras**. Entdeckung und Hinzufügen sind Admin-Sache; andere Benutzer öffnen dieselbe **Kameras**-Seite als schlichte Liste und tippen sich zu jeder Kamera durch.

1. Öffne **Kameras**. Die Suche läuft automatisch. Erscheint eine Kamera nicht, wähle **Neu scannen**.
2. Wähle eine Kamera unter **Entdeckt**. Gib bei Bedarf **Benutzername** und **Passwort** ein (einige Kameras nutzen eine PIN) und wähle **Verbinden**.
3. **Kamera bestätigen** zeigt **Name**, **Raum** und die **Quellen**, die camera.ui aus der Kamera gelesen hat. Passe sie bei Bedarf an.
4. Wähle **Kamera hinzufügen**. Die Kamera wandert zu **Verbundene Kameras**.

Das Augen-Symbol blendet ein Gerät aus, **Ausgeblendete Geräte anzeigen** holt es zurück.
...
Für Kameras, die nicht entdeckt werden, oder um Streams selbst einzurichten, wähle **Kamera hinzufügen** auf der Kameras-Seite.
...
Eine Quelle genügt; ein niedriger aufgelöster Stream entlastet Raster und Wiedergabe. Jede Quelle braucht innerhalb der Kamera einen eigenen Namen; ein bereits vergebener wird mit einem Hinweis unter dem Feld abgelehnt. Ist eine Quelle gespeichert, steht ihr Name fest.
...
- **Gegensprechen deaktivieren.** Entfernt den Rücksprechkanal dieser Quelle, camera.ui fordert ihn bei der Kamera gar nicht erst an. Standardmäßig aus.
