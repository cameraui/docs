...
Der Assistent antwortet in deiner Oberflächensprache, solange **Antwortsprache** nicht auf eine feste Sprache steht. Jedes Werkzeug, das er benutzt hat, erscheint als Karte unter der Antwort, mit Argumenten und Ergebnis.
...
Die Büroklammer hängt eigene Dateien an, Einfügen eines Bildes geht auch: Bilder bis 12 MB, Audio bis 25 MB, Video bis 60 MB, acht Dateien pro Nachricht. Der Assistent gibt sie an die Erkennungs-Plugins weiter. Audio und Video gehen nie an das Sprachmodell, nur an die Plugins. Angehängte Bilder bleiben beim Gespräch, Audio und Video werden einmal analysiert und nicht aufbewahrt.
...
Fahr über eine Nachricht für ihre Aktionen. Eine Frage lässt sich bearbeiten und neu senden, was alles danach verwirft. Eine Antwort lässt sich neu erzeugen, kopieren oder zusammen mit ihrer Frage löschen. **Ab hier abzweigen** kopiert das Gespräch bis zu dieser Stelle in ein neues. Eine gestoppte Antwort bietet **Weiter**. Du kannst die nächste Nachricht tippen, während eine Antwort noch läuft; sie wartet unter dem Textfeld und geht raus, sobald die Antwort fertig ist.

Der Schieberegler-Button im Textfeld öffnet die Einstellungen dieses Gesprächs. **Anweisungen** gelten für jede Antwort darin, etwa „antworte in einem Satz", und wirken auch in langen Gesprächen weiter, in denen frühe Nachrichten aus dem Blickfeld des Modells fallen. **Werkzeuge** listet die Gruppen (camera.ui, die Dokumentation, eine pro Plugin); eine herausgenommene Gruppe sieht das Modell in diesem Gespräch nicht. Bei mehr als einem Modell wählt das Panel auch das **Modell**, und das Gespräch behält es statt des Standards. Eine Kombination aus Modell, Werkzeuggruppen und Anweisungen lässt sich als **Profil** speichern; Profile gehören deinem Konto.

Gespräche liegen pro Nutzer auf dem Server, zusammen mit den Bildern und den Öffnen-Buttons der letzten Werkzeugaufrufe. Eine Antwort, die noch läuft, übersteht eine schlechte Verbindung, einen Reload oder einen zweiten Tab: Der Browser nimmt den Stream dort wieder auf, wo er abgerissen ist, statt das Modell erneut zu fragen, und eine wartende Bestätigung wird nach einem Neustart wieder gezeigt. **Stopp** beendet den Lauf auch auf dem Server, das Schließen des Tabs allein nicht.

## Was sich der Assistent merkt

Fakten, die du erwähnst oder die er sich merken soll („merk dir, dass unser Hund Bruno heißt", „die Kamera Tor zeigt auf die Straße"), bleiben über Gespräche hinweg erhalten und werden dem Modell jedes Mal mitgegeben. Nach jedem Austausch sucht ein kurzer Modellaufruf solche Fakten selbst heraus; Ereignisse und alles Kurzlebige werden nie gespeichert. Ein Modell mit einem Kontextfenster unter 16.000 Tokens lässt diesen Schritt aus und behält nur, was es sich ausdrücklich merken soll. Der Gehirn-Button im Textfeld zeigt die Liste unter **Was der Assistent über dich weiß**, und jeder Eintrag lässt sich vergessen. Admins können die Funktion abschalten.
...
Der camera.ui-Satz ist allgemein: Schnappschüsse, die Sensoren mit ihrem aktuellen Zustand und das Schalten eines Sensors, Push-Benachrichtigungen an die eigenen Geräte, die aktuelle Last der Instanz, der Grundriss, eine Gesundheitsübersicht und drei allgemeine API-Werkzeuge. Diese drei geben ihm die ganze camera.ui-REST-API: Er sucht den Endpunkt, der zur Frage passt, liest ihn mit deinen Rechten und ändert etwas erst nach deiner Bestätigung. Das deckt Plugins, Worker, Sitzungen, Automationen, Nutzer und Zonen ab. Zwei Instanz-Werkzeuge decken ab, was die API nicht kann: Kameras und Sensoren im Netzwerk finden und übernehmen, den Benachrichtigungsverlauf, Logs, Update-Prüfungen, MQTT und Prozesszustände. Zugangsdaten in den Antworten, etwa Stream-Passwörter, werden entfernt, bevor das Modell sie sieht, und Endpunkte für Login, Konfigurationsdateien, Backups und Downloads werden nie angeboten.

Der Assistent kann außerdem ein Erkennungs-Plugin (Objekt, Gesicht, Kennzeichen oder CLIP) auf einen frischen Schnappschuss, eine kurze Folge von Schnappschüssen oder das Bild eines aufgezeichneten Ereignisses anwenden.

Mit dem NVR-Plugin erreicht er deine Aufnahmen: Ereignisse und Episoden in einem Zeitraum, eine Zusammenfassung eines Tages oder einer ganzen Woche, Ereignisse nach Beschreibung gesucht, Ereignisbilder, bekannte Gesichter und wann jemand zuletzt gesehen wurde. Frag nach dem Clip eines Moments und er baut einen mit Download-Button oder schickt ihn als Video-Benachrichtigung aufs Handy. Gesichter, die die Erkennung nicht benennen konnte, lassen sich im Chat benennen, ignorieren oder korrigieren. Er listet außerdem die Kennzeichen eines Zeitraums, sagt dir, wann eine Kamera am meisten zu tun hat und wo im Bild, und zeigt die Strecken, in denen eine Kamera nicht aufgezeichnet hat.[^license]

Der Assistent kennt auch die Nutzer-Dokumentation: Er schlägt nach, wie eine Funktion arbeitet und wo sie in der App liegt, und verweist dorthin, wenn er etwas aus dem Chat heraus nicht kann. Nach Passwörtern fragt er im Chat nie.
...
„Behalte die Einfahrt die nächsten zwei Stunden im Auge" ist dasselbe mit Ende: Der Alarm löscht sich zu dieser Zeit selbst. Jeder Treffer erscheint im Gespräch mit Bild und einem Button zur Aufnahme, und als Push; wenn die Beobachtung endet, schreibt der Assistent eine kurze Zusammenfassung. Dazwischen wird nichts zum Modell gestreamt, es schaut nur auf Bilder, wenn die Kameras etwas aufzeichnen.
